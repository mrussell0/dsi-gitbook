...
> **Instructor Note**: As an alternate to using the official Kaggle site, you can also setup your own class-specific cohort at [inclass.kaggle.com](https://inclass.kaggle.com/). Here you can upload your datasets and define a hold-out test to evaluate against, as well as determine which metric you want to evaluate. This way the only students on the leaderboard are the students in your own class, so they can compete amongst each other (without the demoralizing aspect of competing globally and ranking 900th place!). 
...
> **Instructor Note Follow-up**: If you choose to use the inclass.kaggle method, you will need to update the dataset and evaluation links, accordingly. Note that if the competition you've chosen has not closed yet, you will have to partition the existing training set into both a "training" and "held out" set and only give the students the first part.
